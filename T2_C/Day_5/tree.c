...
tree *new_tree(const unsigned int ntax)
{
    ...
    t->ntax = ntax;
    t->nnodes = 2 * ntax; // 2 *ntax -1: nnodes in a rooted binary tree. But I use a 'dummy'
                          // root so that adds one extra internal node
...
void read_anctable(tree *t, int *anctable)
{
    ...
    for (i = 0; i < t->nnodes; ++i) {
        ...
        // Point the anc pointer of the ith node in t->trnodes to anctable[i]
        // node in t->trnodes
        // anctable[i] -> will need to be used as an index variable itself.
        // i.e. [anctable[i]]
        // Gotcha: what happens at the root
    }
}